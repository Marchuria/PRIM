...
% update: 05/10/2022
% to read files after 2018 that come month by month

function Reading_Gridding_Argo_Rev4(flag)
...
elseif flag==1
    ...
    files = dir([dirin,'RG_ArgoClim_2020*']); %files to process
    
    file2 = dir([dirin,'RG_ArgoClim_S*']); %file where mean sal is saved
    name2 = [dirin,file2.name]; %name of mean salinity file
    
    for k = 1:size(files,1)
    
        name = [dirin,files(k).name]; %name of the file that is being processed
        disp(name)
...
%     lati = double(ncread(name,'/LATITUDE')); %latitude vector
        ...
        sal = double(ncread(name2,'/ARGO_SALINITY_MEAN')); %Argo salinity, pss-78 (-999 missing value), Size: 360x145x58 (lon,lat,pressure)
        ...
        sal = squeeze(sal(:,:,1));
        anom = squeeze(anom(:,:,1,:));
        year = str2num(files(k).name(13:16)); % year from the file name
        mm = str2num(files(k).name(17:18)); % month from the file name
    
        times = [time mm year]; %adding month # and year # to time
...
%         anmly = [nan(25,360,180);anomi;nan(10,360,180)]; %expanding
%         matrices to lat -90 to 90 > for file 2004-2018
        anmly = [nan(25,360);anomi;nan(10,360)]; %for monthly files
...
%         salinity = salinity(:,:,135:end); %saving just march/2015 till
%         the end > for file 2004-2018

        name_file = ['Argo_RG_Salinity_', num2str(year), num2str(mm,'%0.2d')]; %setting name of file
        outputfile = [outDir,'/' name_file ,'.mat']; %setting path for saving
%     save(outputfile,'lat','lon','salinity','time'); %saving output file >
%     for 2004-2018 dataset
        save(outputfile,'lat','lon','salinity','times'); %saving output file
        disp(['Processed file' num2str(k) '-' files(k).name])
